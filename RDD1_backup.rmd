...
### Panel A
```{r}
rdplot(hansen$recidivism, hansen$bac1, p =1, c = 0.08, title = "Recidivism & BAC", y.label = 'Recidivism', x.label = 'BAC', kernel = "triangular", x.lim = c(0,0.2), y.lim = c(0,0.2))
```

### Panel B
```{r}
rdplot(hansen$male, hansen$bac1, p =1, c = 0.08, title = "Male & BAC", y.label = 'Male', x.label = 'BAC', kernel = "triangular", x.lim = c(0,0.2), y.lim = c(0.65,0.9))
```

### Panel C
```{r}
rdplot(hansen$aged, hansen$bac1, p =1, c = 0.08, title = "Age & BAC", y.label = 'Age', x.label = 'BAC', kernel = "triangular", x.lim = c(0,0.2), y.lim = c(30,45))
```

### Panel D
```{r}
rdplot(hansen$white, hansen$bac1, p =1, c = 0.08, title = "White & BAC", y.label = 'White', x.label = 'BAC', kernel = "triangular", x.lim = c(0,0.2), y.lim = c(0.7,0.9))
```
...
```{r}
ggplot(var, aes(bac1, recidivism)) + geom_point(aes(x = bac1, y = recidivism), data = hansen)
  +geom_vline(xintercept = 0.08, colour = "blue", linetype = 2) +
  stat_smooth(aes(bac1, recidivism, group = gg_group2), method = "lm", formula = y ~ x + I(x^2))
  +labels(x = "BAC", title = "All Ofenders")
```
